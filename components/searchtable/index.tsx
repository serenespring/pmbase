/** 搜索table author: serenespring date: 2021-01-10 */
...
const SearchTable = (props: any) => {
  ...
  const getFields = () => {
    const newSearchColumns = expand
      ? searchColumns
      : searchColumns.length > 6
      ? searchColumns.slice(0, 6)
      : searchColumns;
    const children = [];
    for (let i = 0; i < newSearchColumns.length; i++) {
      switch (newSearchColumns[i].valueType) {
        case 'select':
          children.push(
            <Col span={8} key={i}>
              <Form.Item name={newSearchColumns[i]?.dataIndex} label={newSearchColumns[i]?.title}>
                <Select
  ...
  return (
    <Card>
      ...
      <div style={{ marginTop: '20px' }}>
        <div style={{ textAlign: 'right' }}>{actionRender(selectedRowKeys)}</div>
        {isRowSelection ? (
          <Table
            columns={columns}
