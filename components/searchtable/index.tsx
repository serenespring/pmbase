...
const SearchTable = (props: any) => {
  ...
  const getFields = () => {
    ...
    for (let i = 0; i < newSearchColumns.length; i++) {
      ...
      switch (searchColumns[i].valueType) {
        case "select":
          children.push(
            <Col span={8} key={i}>
              <Form.Item
                name={searchColumns[i]?.dataIndex}
                label={searchColumns[i]?.title}
              >
                <Select
                  placeholder={`请选择${searchColumns[i]?.title}`}
                ...
                >
                  {
                    searchColumns[i]?.valueData.map((item1: any) => (
          ...
          children.push(
            <Col span={8} key={i}>
              <Form.Item
                ...
                label={searchColumns[i]?.title}
              >
                <Input placeholder={`请输入${searchColumns[i]?.title}`} />
              </Form.Item>
            </Col>,
          );
      }
